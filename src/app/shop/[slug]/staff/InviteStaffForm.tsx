...
export function InviteStaffForm({
    ...
    const submit = (e: React.FormEvent) => {
        ...
        start(async () => {
            try {
                ...
                if ((prof as any).kind !== 'user') {
                    setErr(`@${target} is not a user profile`);
                    return;
                }
                await setStaffRole((prof as any).id, shopId, slug, role);
                setHandle('');
            } catch (e: any) {
    ...
    return (
        <form onSubmit={submit} className="admin-form" style={{ marginTop: 24 }}>
            <div className="admin-form-label">INVITE STAFF</div>
            <input
                type="text"
                value={handle}
